...
public class ScrollHandler {
    ...
    public boolean mouseScrolled(double maxScroll, double amount) {
        if (this.getMaxScroll() > 0) {
            ...
            double scroll = Math.clamp(this.getScroll() - finalScroll, 0, this.getMaxScroll());
    ...
    public ScrollHandler setScroll(double scroll) {
        this.scroll = Math.clamp(scroll, 0, this.height - this.visibleHeight);
    ...
    public float getInterpolatedScroll(float partialTicks) {
        return (float) org.joml.Math.lerp(partialTicks, this.lastScroll, this.scroll);
    }
